...
def main():
    ...
    k_values = [int(k) for k in args.report.split(",")]
    all_results = {}
    ...
    for d in args.dirs:
        ...
        dir_results = {
            ...
            "excess_code": excess_code,
            "excess_code_se": excess_code_se,
            "mean_median_coverage": mean_median_coverage,
        }

        for k in k_values:
            if k <= min_completions:
                pass_k = np.round(np.mean([estimator(r["n"], r["c"], k)
                                       for r in results]), 6)
                dir_results[f"pass@{k}"] = float(pass_k)
                
                print(
                    f"{name},{k},{pass_k},{num_problems},{min_completions},{max_completions},{excess_code},{excess_code_se},{mean_median_coverage}")

        all_results[name] = dir_results
